...
- Option drop: remove CreateJob column from the dataset
"""
...
def preprocess_createjob_option_trees(
	df: pd.DataFrame,
	source_col: str = "CreateJob",
) -> pd.DataFrame:
	"""Tree option for CreateJob (clean numeric values, keep natural units)."""
	if source_col not in df.columns:
		raise KeyError(f"Column '{source_col}' not found in DataFrame")

	result = df.copy()
	result[source_col] = _to_numeric_createjob(result[source_col])
	return result


def preprocess_createjob_option_drop(
	df: pd.DataFrame,
	source_col: str = "CreateJob",
) -> pd.DataFrame:
	"""Drop option for CreateJob (remove the column)."""
	if source_col not in df.columns:
		raise KeyError(f"Column '{source_col}' not found in DataFrame")
...
def preprocess_createjob(
	...
	option : str
		...
		"drop" to remove CreateJob from the dataset.
	source_col : str
	...
	if option_upper == "TREES":
		return preprocess_createjob_option_trees(df=df, source_col=source_col)
	if option_upper == "DROP":
		return preprocess_createjob_option_drop(df=df, source_col=source_col)

	raise ValueError("option must be 'A', 'B', 'C', 'trees', or 'drop'")
